...
def test_menu_item_requires_milk_foam(coffee_item):
    assert coffee_item.requires_milk_foam is True


def test_menu_initialization_empty():
    menu = Menu(menu_items=set())
    assert len(menu._drinks) == 0
    assert len(menu._food) == 0


def test_menu_initialization_with_items(coffee_item, tea_item):
...
def test_menu_add_drink(coffee_item):
    menu = Menu(set())
...
def test_menu_remove_drink(coffee_item):
...
def test_menu_str_representation(coffee_item):
    menu = Menu({coffee_item})
    str_repr = str(menu)
...
def test_menu_init_without_arguments_should_handle_none():
    menu = Menu()
    assert len(menu._drinks) == 0
